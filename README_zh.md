...
[Autumn](https://useautumn.com) 是一个开源层，位于 Stripe 和您的应用程序之间，让您只需几行代码即可创建任何定价模型并嵌入它。在 Autumn 上，您可以构建：
- 订阅
- 信用系统和充值
- 基于使用量的模型和超额费用
- 大客户的自定义计划
...
**云端**：使用 Autumn 的最快方式是通过我们的[云服务](https://app.useautumn.com)。
...
> ⚠️ 要登录，请在登录页面输入电子邮件，OTP 应该出现在您的控制台/终端。通常，我们使用 Resend 发送 OTP 电子邮件或 Google OAuth —— 这些可以通过在 `server/.env` 中提供您的凭据来设置

> ℹ️ 我们的设置脚本初始化所需的 env 变量，并（可选）一个 Supabase 实例。如果您想使用自己的 Postgres 实例，您可以这样做 —— 只需将连接字符串粘贴到 `server/.env` 中的 `DATABASE_URL` env 变量

## 故障排除

如果在运行 `bun setup` 时遇到 `SyntaxError: Unexpected end of JSON input` 错误，您可能需要先清除数据库表。这是运行数据库迁移多次时可能发生的[已知问题](https://github.com/drizzle-team/drizzle-orm/issues/4529)。
...
不仅仅是支付：它是构建权限管理、计量、使用限制与 cron 作业，并将其连接到升级、降级、取消和失败支付状态。竞争条件、边缘情况和其他错误会减慢您的速度。

**2️⃣ 计费和应用逻辑应该解耦**

成长中的公司经常迭代定价：提高价格、实验信用或为新功能收费。DB 迁移、重建应用内流程、内部仪表板用于自定义定价和对旧定价的用户进行祖父条款是一场噩梦。

## 它是如何工作的

首先，在仪表板上创建您的产品和计划。我们支持**任何**定价模型。我们看到的一些流行模型包括：

1. **使用量和超额** ⚡：设置实时使用限制并选择何时重置。如果用户超额，则向他们收费。
2. **信用** 💰：用户可以访问货币或任意信用，许多功能可以从中提取
3. **基于席位的与每席限制** 👥：为客户的用户（或其他实体）计费
4. **预付** 💳：让用户预先购买固定数量的功能，随着时间的推移使用

接下来，所有您的计费逻辑都可以通过 3 个函数实现：

1. `/attach`：一个函数调用处理所有购买流程。我们返回 Stripe Checkout URL，或处理升级/降级。
...
**贡献** 🤝：如果您有兴趣贡献，您可以查看我们的指南[这里](/.github/CONTRIBUTING.md)。所有类型的帮助都受到赞赏 :)

**支持** 💬：如果您需要任何类型的支持，我们通常在我们的[Discord 频道](https://discord.gg/STqxY92zuS)上最响应，但也可以通过电子邮件联系我们 `hey@useautumn.com`！
