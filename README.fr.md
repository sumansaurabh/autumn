...
Tout cela sans avoir à gérer les webhooks, les montées/descentes de gamme, les annulations ou les échecs de paiement.


## Démarrage

**Cloud** : La façon la plus rapide de commencer à utiliser Autumn est via notre [service cloud](https://app.useautumn.com).

**Auto-hébergé** : Si vous souhaitez héberger Autumn vous-même :
...
> ⚠️ Pour vous connecter, entrez un email sur la page de connexion, et un code OTP devrait apparaître dans votre console / terminal. Normalement, nous utilisons Resend pour envoyer un OTP par email ou Google OAuth -- ceux-ci peuvent être configurés en fournissant vos identifiants dans `server/.env`
...
Plus que des paiements : il s'agit de construire la gestion des permissions, la mesure, les limites d'utilisation avec des tâches cron, et de les connecter aux états de montée de gamme, descente de gamme, annulation et échecs de paiement. Les conditions de course, les cas particuliers et autres bugs vous ralentiront.

**2️⃣ La logique de facturation et d'application doit être découplée**

Les entreprises en croissance itèrent souvent sur la tarification : augmentation des prix, expérimentation avec les crédits ou facturation d'une nouvelle fonctionnalité. Les migrations de base de données, la reconstruction des flux dans l'application, les tableaux de bord internes pour la tarification personnalisée et le maintien des utilisateurs sur les anciens prix sont un cauchemar.
...
1. `/attach` : Un appel de fonction pour tous les flux d'achat. Nous renvoyons une URL Stripe Checkout, ou gérons une montée/descente de gamme.
...
!data.allowed && alert("Limite AI atteinte")
```
...
**Support** 💬 : Si vous avez besoin de tout type de support, nous sommes généralement plus réactifs sur notre [canal Discord](https://discord.gg/STqxY92zuS), mais n'hésitez pas à nous envoyer un email à `hey@useautumn.com` aussi !



<!-- ## Félicitations !

Vous avez intégré un système de facturation complet dans votre application en quelques minutes. Vous pouvez apporter les modifications de modèle de tarification dont vous avez besoin, ou gérer des plans personnalisés sans avoir à modifier votre base de code.

N'hésitez pas à héberger Autumn vous-même, ou à utiliser notre version hébergée sur https://useautumn.com. Et faites-nous savoir toutes vos questions, réflexions ou commentaires à hey@useautumn.com. -->
