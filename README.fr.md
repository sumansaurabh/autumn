...
[Autumn](https://useautumn.com) est une couche open-source entre Stripe et votre application, vous permettant de créer n'importe quel modèle de tarification et de l'intégrer avec quelques lignes de code. Sur Autumn, vous pouvez construire :
- Des abonnements
...
Tout cela sans avoir à gérer les webhooks, les mises à niveau/déclassements, les annulations ou les échecs de paiement.
...
Plus que des paiements : il s'agit de construire la gestion des permissions, la mesure, les limites d'utilisation avec des tâches cron, et de les connecter aux états de mise à niveau, déclassement, annulation et échecs de paiement. Les conditions de concurrence, les cas limites et autres bugs vous ralentiront.

**2️⃣ La facturation et la logique applicative doivent être découplées**

Les entreprises en croissance itèrent souvent sur les prix : augmentation des prix, expérimentation avec des crédits ou facturation d'une nouvelle fonctionnalité. Les migrations de base de données, la reconstruction des flux dans l'application, les tableaux de bord internes pour la tarification personnalisée et le maintien des anciens tarifs pour les utilisateurs existants est un cauchemar.
...
1. **Utilisation & Dépassement** ⚡ : définissez des limites d'utilisation en temps réel et choisissez quand elles se réinitialisent. Facturez les utilisateurs s'ils dépassent.
2. **Crédits** 💰 : les utilisateurs peuvent accéder à des crédits monétaires ou arbitraires auxquels de nombreuses fonctionnalités peuvent puiser
3. **Basé sur les sièges avec limites par siège** 👥 : facturez les clients pour leurs utilisateurs (ou autres entités)
4. **Paiement anticipé** 💳 : laissez les utilisateurs acheter une quantité fixe d'une fonctionnalité à l'avance, qui est utilisée au fil du temps


Ensuite, toute votre logique de facturation peut être implémentée avec seulement 3 fonctions :

1. `/attach` : Un seul appel de fonction pour tous les flux d'achat. Nous renvoyons une URL Stripe Checkout, ou gérons une mise à niveau/déclassement.
...
2. `/check` : Vérifiez si un client a accès à un produit, une fonctionnalité ou l'utilisation restante.
```ts
...
**Contribuer** 🤝 : Si vous souhaitez contribuer, vous pouvez consulter notre guide [ici](/.github/CONTRIBUTING.md). Tous les types d'aide sont appréciés :)

**Support** 💬 : Si vous avez besoin d'un quelconque type de support, nous sommes généralement plus réactifs sur notre [canal Discord](https://discord.gg/STqxY92zuS), mais n'hésitez pas à nous envoyer un email à `hey@useautumn.com` aussi !



<!-- ## Félicitations !

Vous avez intégré un système de facturation complet dans votre application en quelques minutes. Vous pouvez effectuer toutes les modifications de modèle de tarification dont vous avez besoin, ou gérer des plans personnalisés sans avoir besoin de modifier votre base de code.
