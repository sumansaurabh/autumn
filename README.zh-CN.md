...
[![Documentation](https://img.shields.io/badge/文档-📕-blue)](https://docs.useautumn.com)
[![Ask DeepWiki](https://deepwiki.com/badge.svg)](https://deepwiki.com/useautumn/autumn)

[Autumn](https://useautumn.com) 是一个开源的 Stripe 和您的应用程序之间的中间层，允许您创建任何定价模型并通过几行代码嵌入。在 Autumn 上您可以构建：
- 订阅制
- 积分系统和充值
- 基于用量的模型和超额计费
- 为大客户定制的计划
...
完成！您应该能在 `http://localhost:3000` 看到 Autumn 仪表板。

> ⚠️ 要登录，请在登录页面输入电子邮件，OTP 验证码将出现在您的控制台/终端中。通常，我们使用 Resend 发送 OTP 电子邮件或 Google OAuth——可以通过在 `server/.env` 中提供您的凭据来设置这些功能

> ℹ️ 我们的设置脚本会初始化所需的环境变量和（可选的）Supabase 实例。如果您想使用自己的 Postgres 实例，也可以这样做——只需将连接字符串粘贴到 `server/.env` 中的 `DATABASE_URL` 环境变量中即可

## 故障排除

如果您在之前运行过 `bun setup` 后再次运行时遇到 `SyntaxError: Unexpected end of JSON input` 错误，您可能需要先清除数据库表。这是一个[已知问题](https://github.com/drizzle-team/drizzle-orm/issues/4529)，可能在多次运行数据库迁移时发生。
...
不仅仅是支付：它需要构建权限管理、计量、使用限制与定时任务，并将其连接到升级、降级、取消和支付失败状态。竞态条件、边缘情况和其他 bug 会拖慢您的进度。

**2️⃣ 计费和应用逻辑应该解耦**

成长型公司经常迭代定价：提高价格、尝试积分制或为新功能收费。数据库迁移、重建应用内流程、为定制定价和老用户保留旧定价的内部仪表板是一场噩梦。


## 工作原理
首先，在仪表板上创建您的产品和计划。我们支持**任何**定价模型。一些我们看到的流行模型包括：

1. **用量和超额** ⚡：设置实时使用限制并选择何时重置。如果用户超额则收费。
2. **积分** 💰：用户可以访问货币或任意积分，许多功能可以从中扣除
3. **基于席位的每席位限制** 👥：根据用户（或其他实体）向客户收费
...
**支持** 💬：如果您需要任何类型的支持，我们通常在 [Discord 频道](https://discord.gg/STqxY92zuS)上响应最快，但也欢迎通过电子邮件 `hey@useautumn.com` 联系我们！
